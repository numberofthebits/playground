...
static int load_texture_data(const char *file_path, ImageMeta *meta,
                             void **data) {
...
static int load_shader_source(const FileSystemListResult *result,
                              void *user_data) {
  if (!result) {
    LOG_EXIT("Invalid file system list result");
  }
  ...
  if (shader_index < 0) {
    LOG_EXIT("Failed to find shader index from shader type '%d', file ext '%s'",
             shader_type, result->extension);
  }
...
static void load_shaders(struct Assets *assets) {
  ...
  file_system_list("./assets/shaders/", "tilemap.*", &load_shader_source,
                   &context);
  ...
  file_system_list("./assets/shaders/", "unit.*", &load_shader_source,
                   &context);
  ...
  file_system_list("./assets/shaders/", "collision_debug.*",
                   &load_shader_source, &context);
...
static int assets_make_file_path(const char *path, const char *file_name,
...
int assets_load_asset(struct Assets *assets, AssetId id, void **data,
  ...
  void *asset_ptr = 0;
  int found_asset_id =
      hash_map_get(&assets->textures, &id, sizeof(id), &asset_ptr);
  if (!found_asset_id) {
    return 0;
  }
  ...
  if (!assets_make_file_path(asset->file_path, asset->name.name, buf,
                             sizeof(buf))) {
    LOG_ERROR("Failed to make file path for path '%s' file '%s': Concat buffer "
              "size=%d",
              asset->file_path, asset->name, MAKE_FILE_PATH_BUF_MAX);
    return 0;
  }
...
/* void assets_add_texture(struct Assets* assets, TextureHandle* handle, const
 * char* asset_name, ImageMeta* meta) { */
...
/* void assets_add_tiled_texture(struct Assets* assets, TextureHandle* handle,
 * const char* asset_name, ImageMeta* meta, uint16_t tiles_x, uint16_t tiles_y)
 * { */
/*     LOG_INFO("Add tiled texture '%s'", asset_name); */
/*     AssetTiledTexture texture; */
/*     assets_add_texture_init_shared((AssetTexture*)&texture, handle,
 * asset_name, meta); */
