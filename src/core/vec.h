...
#define VEC_PUSH_T(vec, type, value)                                    \
    {                                                                   \
        if ((vec)->size > (vec)->capacity) { LOG_EXIT("What??"); }      \
...
#define VEC_POP_T(vec)                          \
    (vec)->size -= 1;

#define VEC_GET_T_PTR(vec, type, index)         \
    (((type*)(vec)->storage.ptr) + index)

#define VEC_GET_T(vec, type, index)             \
    *(((type*)(vec)->storage.ptr) + index)
...
#define VEC_RESERVE_T(vec, type, count)         \
    VEC_RESERVE((vec), sizeof(type), count);    \
...
#define VEC_RESIZE_T(vec, type, count)          \
    VEC_RESIZE((vec), sizeof(type), (count));   \

#define VEC_ITER_BEGIN_T(vec, type)             \
    (type*)((vec)->storage.ptr)
