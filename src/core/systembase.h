...
/* SystemBase defines invariants for concrete systems such that
   ...
   Systembase describes which entities are interesting for a concrete system, and tracks
   which entities are registered in the system.
   
   Each system is identified by a bit flag 'flag'. This is just an integer, because
   we don't want anything in core to know about concrete systems.
 */
struct SystemBase {
  int flag; /*SystemBit*/
  ...
  //       each concrete system to take the parameters it needs to update, instead
  //       of the update callback parameters becoming the set of all arguments of
  //       all concrete update functions.
