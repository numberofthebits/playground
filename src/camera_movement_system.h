...
static inline Vec3f clamp_camera_pos(Vec3f* object_position, Vec2u8 world_size, Vec2f* area) {
...
static inline void camera_movement_system_update(Registry* reg, struct SystemBase* base, size_t frame_nr) {
    (void)reg;
    (void)base;
    (void)frame_nr;
    ...
    for (int i = 0; i < base->entities.size || i < 1; ++i) {
        Entity e = *(ptr + i);
        TransformComponent* tc = PoolGetComponent(pool, TransformComponent, e.index);
        CameraUpdated data;
        data.pos = clamp_camera_pos(&tc->pos,
                                    camera_movement_system->world_size,
